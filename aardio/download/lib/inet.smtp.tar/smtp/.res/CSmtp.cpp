...
CSmtp::CSmtp()
{
	...
	m_eLastErrorCode = ECSmtp::CSMTP_NO_ERROR;
	...
	if (WSAStartup(wVer,&wsaData) != NO_ERROR){
		m_eLastErrorCode = (ECSmtp::WSA_STARTUP);
		return;
	}
	if (LOBYTE( wsaData.wVersion ) != 2 || HIBYTE( wsaData.wVersion ) != 2 ) 
	{
		WSACleanup();
		m_eLastErrorCode = (ECSmtp::WSA_VER);
		return;
	}
#endif
	...
	if(gethostname((char *) &hostname, 255) == SOCKET_ERROR) {
		m_eLastErrorCode = (ECSmtp::WSA_HOSTNAME);
		return;
	}
	m_sLocalHostName = hostname;
	
	if((RecvBuf = new char[BUFFER_SIZE]) == NULL){
		m_eLastErrorCode = (ECSmtp::LACK_OF_MEMORY);
		return;
	}
...
{	
	m_eLastErrorCode = ECSmtp::CSMTP_NO_ERROR;
	if(!email){
		m_eLastErrorCode = (ECSmtp::UNDEF_RECIPIENT_MAIL);
		return;
	}
	Recipient recipient;
...
void CSmtp::DelMsgLine(unsigned int Line)
{
	...
	if(Line >= MsgBody.size()){
		m_eLastErrorCode = (ECSmtp::OUT_OF_MSG_RANGE);
		return;
	}
...
void CSmtp::Send()
{
	...
	if(hSocket==INVALID_SOCKET)
	{
		...
		{
			m_eLastErrorCode = (ECSmtp::WSA_INVALID_SOCKET);
			return;
		}
	}
	...
	{
		m_sLastRequest = SendBuf;
		m_sLastResponse = RecvBuf; 
		m_iLastResponseCode = m_iResponseCode;

		if(hFile) fclose(hFile);
		if(FileBuf) delete[] FileBuf;
		DisconnectRemoteServer();
		m_eLastErrorCode =  e.GetErrorNum();
		return;
	}
								...
								const char* password/*=NULL*/)
{
	...
	catch(const ECSmtp& e)
	{
		...
		if( ( m_iResponseCode == 530 ) || ( RecvBuf[0]=='5' && RecvBuf[1]=='3' && RecvBuf[2]=='0') )
			m_bConnected=false;
		DisconnectRemoteServer();
...
{
	m_bReadReceipt = requestReceipt?true:false;
}
...
{
	m_bAuthenticate = Login ? true : false;
	m_sLogin = Login;
}
...
void CSmtp::SetSMTPServer(const char* SrvName, const unsigned short SrvPort, BOOL authenticate)
{
	...
	m_bAuthenticate =  authenticate?true:false;
}
...
void CSmtp::ReceiveResponse(Command_Entry* pEntry)
{
			...
			if(offset + 1 < len) // we found a line
			{
				...
				if(offset - begin >= 5)
				{
					if(isdigit(line[begin]) && isdigit(line[begin+1]) && isdigit(line[begin+2]))
					{
						...
						{
							m_iResponseCode = (line[begin]-'0')*100 + (line[begin+1]-'0')*10 + line[begin+2]-'0';
							bFinish = true;
							break;
						}
					}
	...
	if(m_iResponseCode != pEntry->valid_reply_code)
	{
