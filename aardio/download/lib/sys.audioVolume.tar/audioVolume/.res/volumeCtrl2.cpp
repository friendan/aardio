...
extern "C" __declspec(dllexport) IAudioMeterInformation * __cdecl GetMeterInfomation(CVolumeCtrl2* pCtrl) {
	return pCtrl->GetMeterInfomation();
}
...
IAudioMeterInformation * CVolumeCtrl2::GetMeterInfomation()
{
	if(m_bVistaLater){
		m_pMeterInfomation->AddRef();
		return m_pMeterInfomation;
	}
	return NULL;
}
...
BOOL CVolumeCtrl2::InitEndPointVistaLater()
{
	...
	hr = m_pDevice->Activate(__uuidof(IAudioMeterInformation),
		CLSCTX_ALL, NULL, (void**)&m_pMeterInfomation);
