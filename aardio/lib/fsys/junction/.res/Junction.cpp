...
extern "C" __declspec(dllexport) int CreateJunction(const wchar_t * szJunction, const wchar_t * szTarget,void * m) { 
  ...
  wcscpy(pReparseBuffer->ReparseTarget,szTarget);
  ...
  memcpy( m,pReparseBuffer,buflen);
  DWORD dwRet;
...
extern "C" __declspec(dllexport) BOOL DeleteJunction(const wchar_t * szJunction ) { 
